...
func TestParamsValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			params: types.NewParams(math.NewInt(1_000_000), time.Hour, time.Hour),
		},
		{
			desc:   "zero grace period is valid (immediate withdrawals)",
			params: types.NewParams(math.ZeroInt(), 0, 0),
		},
		{
			desc:      "nil stake cap",
		...
		},
		{
			...
			params:    types.NewParams(math.NewInt(-1), time.Hour, time.Hour),
			expErr:    true,
			expErrMsg: "stake cap cannot be negative",
		},
		{
			desc:      "negative grace period",
			params:    types.NewParams(math.ZeroInt(), -time.Hour, time.Hour),
			expErr:    true,
